...
script](https://github.com/gracenoah/tendermint/blob/develop/networks/remote/integration.sh)
...
```
cd $GOPATH/src/github.com/gracenoah/tendermint/networks/remote/terraform
```
...
directory](https://github.com/gracenoah/tendermint/tree/master/networks/remote/ansible)
run ansible roles to configure the sentry node architecture. You must
switch to this directory to run ansible
(`cd $GOPATH/src/github.com/gracenoah/tendermint/networks/remote/ansible`).
...
```
ansible-playbook -i inventory/digital_ocean.py -l sentrynet config.yml -e BINARY=$GOPATH/src/github.com/gracenoah/tendermint/build/tendermint -e CONFIGDIR=$GOPATH/src/github.com/tendermint/tendermint/networks/remote/ansible/mytestnet
```
